        ...
        if conn is not None:
            try:
                with conn.cursor() as cursor:
                    cursor.execute('SELECT * FROM coindesk_data WHERE id = %s', (id,))
                    row = cursor.fetchone()
                    return row
            except err.MySQLError as e:
...
def getBalanceAsset(id):
    with create_connection() as conn:
        if conn is not None:
            try:
                with conn.cursor() as cursor:
                    cursor.execute('SELECT * FROM balanceAsset2 WHERE id = %s', (id,))
                    row = cursor.fetchone()
                    return row
            except err.MySQLError as e:
                logger.error(f"Database query failed: {e}")
    ...
    with create_connection() as conn:
        if conn:
            try:
                with conn.cursor() as cursor:
                    cursor.execute('''
                        INSERT INTO balanceAsset2 (timestamp, usdtBalance, bitcoinBalance, bitcoinValue, assetValue)
                        VALUES (%s, %s, %s, %s, %s)
