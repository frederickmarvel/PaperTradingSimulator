...
def insertData2():
    xbx_url = "https://cdi-website-three.vercel.app/indices/api/v1/trend-indicator?symbol=etx&history=true"
    try:
        response = requests.get(xbx_url)
        response.raise_for_status()  
        data = response.json()['data']
        for item in data:
            try:
                cursor.execute('''
                    INSERT INTO coindesk_data2 (timestamp, index_level, trend_score) VALUES (%s, %s, %s)
                    ON DUPLICATE KEY UPDATE
                    index_level=VALUES(index_level),
                    trend_score=VALUES(trend_score)
                ''', (item['timestamp'], item['index_level'], item['trend_score']))
