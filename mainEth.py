...
def getBalanceAsset(id):
    with create_connection() as conn:
        if conn is not None:
            try:
                with conn.cursor() as cursor:
                    cursor.execute('SELECT * FROM balanceAssetETH WHERE id = %s', (id,))
                    row = cursor.fetchone()
                    return row
            except err.MySQLError as e:
                logger.error(f"Database query failed: {e}")
    ...
    with create_connection() as conn:
        if conn:
            try:
                with conn.cursor() as cursor:
                    cursor.execute('''
                        INSERT INTO balanceAssetETH (timestamp, usdtBalance, ethereumBalance, ethereumValue, assetValue)
                        VALUES (%s, %s, %s, %s, %s)
...
rebalancePortfolio(1,0.5, 1535486400, 100000, 0)   
